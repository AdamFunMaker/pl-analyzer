...
export class AnalysisService {
    ...
    async getOverview(interval, period) {
        try {
            if (period[0]) {
                const criteria = `${interval === "Monthly" ? period[1] ? "\`date\` BETWEEN ? AND ?" : "year = ? AND month = ?" : period[1] ? "year BETWEEN ? AND ?" : "year = ?"}`;
                const sql = formatSQLString(`SELECT year, ${interval === "Monthly" ? "month, buy_weight, buy_price, buy_average_price, sell_weight, sell_price, sell_average_price, buy_weight - sell_weight AS stock_weight, salary, expenses, petty_cash, profit_loss" : "SUM(buy_weight) AS buy_weight, SUM(buy_price) AS buy_price, ifnull(SUM(buy_price) / SUM(buy_weight), 0) AS buy_average_price, SUM(sell_weight) AS sell_weight, SUM(sell_price) AS sell_price, ifnull(SUM(sell_price) / SUM(sell_weight), 0) AS sell_average_price, ifnull(SUM(buy_weight) - SUM(sell_weight), 0) AS stock_weight, SUM(salary) AS salary, SUM(expenses) AS expenses, SUM(petty_cash) AS petty_cash, SUM(profit_loss) AS profit_loss"} FROM overview WHERE ${criteria} ${interval === "Yearly" ? "GROUP BY year ORDER BY year DESC" : ""}`, interval === "Monthly" ? period[1] ? period : [period[0].getFullYear(), period[0].getMonth() + 1] : period[1] ? period.map(date => date.getFullYear()) : period[0].getFullYear());
    ...
    async getOverviewBreakdown(interval, period) {
        try {
            if (period[0]) {
                const criteria = `${interval === "Monthly" ? period[1] ? "\`date\` BETWEEN ? AND ?" : "year = ? AND month = ?" : period[1] ? "year BETWEEN ? AND ?" : "year = ?"}`;
                const sql = formatSQLString(`SELECT year, ${interval === "Monthly" ? "month, category, buy_weight, buy_price, buy_average_price, sell_weight, sell_price, sell_average_price, buy_weight - sell_weight AS stock_weight" : "category, SUM(buy_weight) AS buy_weight, SUM(buy_price) AS buy_price, ifnull(SUM(buy_price) / SUM(buy_weight), 0) AS buy_average_price, SUM(sell_weight) AS sell_weight, SUM(sell_price) AS sell_price, ifnull(SUM(sell_price) / SUM(sell_weight), 0) AS sell_average_price, ifnull(SUM(buy_weight) - SUM(sell_weight), 0) AS stock_weight"} FROM overview_categories WHERE ${criteria} ${interval === "Yearly" ? "GROUP BY year, category ORDER BY year DESC" : ""}`, interval === "Monthly" ? period[1] ? period : [period[0].getFullYear(), period[0].getMonth() + 1] : period[1] ? period.map(date => date.getFullYear()) : period[0].getFullYear());
                const result = await database.select(sql);
                return {success: true, data: result}
            } else {
                return {success: true, data: []}
