...
<template>
    <form class="mx-8 flex flex-col gap-4" @submit.prevent="dialogRef.data.hasSubmitted = true">
        <article v-for="field in fields" class="flex flex-col gap-2">
            ...
            <DatePicker v-if="field.type === 'date'" :inputId="field.id" v-model="newRecord[field.id]" :minDate="field.minimum" :maxDate="field.maximum" inline showButtonBar hideOnRangeSelection :invalid="validateField(field)" fluid autofocus></DatePicker>
            <DatePicker v-if="field.type === 'month'" :inputId="field.id" v-model="newRecord[field.id]" view="month" :minDate="field.minimum" :maxDate="field.maximum" inline showButtonBar hideOnRangeSelection :invalid="validateField(field)" fluid autofocus></DatePicker>
