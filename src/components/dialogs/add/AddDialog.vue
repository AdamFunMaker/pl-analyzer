...
<template>
    <form class="mx-8 flex flex-col gap-4" @submit.prevent="dialogRef.data.hasSubmitted = true">
        <article v-for="(field, index) in fields" class="flex flex-col gap-2">
            ...
            <InputText v-if="field.type === 'text'" :id="field.id" v-model.trim="newRecord[field.id]" :placeholder="field.placeholder" :invalid="validateField(field)" fluid></InputText>
            <InputNumber v-if="field.type === 'number'" :id="field.id" v-model="newRecord[field.id]" :min="field.minimum" :max="field.maximum" :step="field.step" :minFractionDigits="field.minFractionDigits" :maxFractionDigits="field.maxFractionDigits" :prefix="field.prefix" :suffix="field.suffix" :placeholder="field.placeholder" :allowEmpty="!field.required" :invalid="validateField(field)" fluid showButtons highlightOnFocus></InputNumber>
            ...
            <DatePicker v-if="field.type === 'month'" :id="field.id" v-model="newRecord[field.id]" view="month" dateFormat="mm/yy" :minDate="field.minimum" :maxDate="field.maximum" fluid inline showButtonBar :invalid="validateField(field)"></DatePicker>
