        ...
        newData.map(record => record.category).filter((record, index, records) => index === records.findIndex(category => category === record)).forEach(category => {
            chartsData.value[category] = {
                ...
                datasets: [
                    {
                        label: "Buy",
                        data: newData.filter(record => record.category === category).map(record => record.buy_price),
                        backgroundColor: documentStyle.getPropertyValue("--p-blue-500"),
                        borderColor: documentStyle.getPropertyValue("--p-blue-500")
                    },
                    {
                        label: "Sell",
                        data: newData.filter(record => record.category === category).map(record => record.sell_price),
